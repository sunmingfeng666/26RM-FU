...
{
  SetRangeColor(DIN5_START_INDEX, DIN5_END_INDEX, DIN5_RED_LEVEL, 0, 0);
}
...
static uint8_t ReadPressedHitButtonRaw(void)
{
  for (uint8_t i = 0; i < (sizeof(kHitButtons) / sizeof(kHitButtons[0])); ++i)
  {
    if (HAL_GPIO_ReadPin(kHitButtons[i].port, kHitButtons[i].pin) == GPIO_PIN_RESET)
    {
      switch (i)
      {
        case 0u: return HIT_KEY_PB10;
        case 1u: return HIT_KEY_PB11;
        case 2u: return HIT_KEY_PB12;
        case 3u: return HIT_KEY_PB13;
        case 4u: return HIT_KEY_PB14;
        case 5u: return HIT_KEY_PA4;
        case 6u: return HIT_KEY_PB6;
        case 7u: return HIT_KEY_PB7;
        case 8u: return HIT_KEY_PB15;
        default: return 0u;
      }
    }
  }
...
int main(void)
{
  ...
  while (1)
  {
    uint8_t can_active = a;
    uint8_t pressed_key = GetPressedHitButton();
    uint8_t button_pressed = (pressed_key != 0u);

    if (can_active != last_can_active)
    {
      last_can_active = can_active;

      if (can_active)
      {
        hit_done = 0;
        flow_hit_mode = 0;
        flow_hit_din0_start = 0;
        flow_hit_din0_end = 0;
    ...
    if (can_active && !hit_done && button_pressed)
    {
      if (key_lock == 0)
      {
        key_lock = 1;
        hit_done = 1;
        RUI_F_CAN_SEDN_UNION(&hcan, HIT_FEEDBACK_ID, tx_data);
    ...
    if (can_active && (flow_hit_mode || !hit_done) && ((HAL_GetTick() - last_flow_tick) >= FLOW_DELAY_MS))
    {
      last_flow_tick = HAL_GetTick();
