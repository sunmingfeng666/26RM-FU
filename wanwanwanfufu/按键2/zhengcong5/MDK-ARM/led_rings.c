...
static uint32_t g_cycle_tick = 0;
static uint8_t g_cycle_mode = 0;
...
static const uint16_t g_pending_pattern_leds[] = {
    STRIP1_LED(250), STRIP1_LED(251), STRIP1_LED(252), STRIP1_LED(253),
    STRIP1_LED(254), STRIP1_LED(255), STRIP1_LED(256), STRIP1_LED(257),
    STRIP1_LED(258), STRIP1_LED(259), STRIP1_LED(260), STRIP1_LED(261)
};

static uint16_t ArmorLED_GetTargetPatternCount(void)
{
    return (uint16_t)(sizeof(g_target_pattern_leds) / sizeof(g_target_pattern_leds[0]));
}

static uint16_t ArmorLED_GetPendingPatternCount(void)
{
    return (uint16_t)(sizeof(g_pending_pattern_leds) / sizeof(g_pending_pattern_leds[0]));
}
...
void ArmorLED_ShowIndices(const uint16_t *indices, uint16_t count, uint8_t r, uint8_t g, uint8_t b)
{
    ...
    for (uint16_t i = 0; i < count; ++i) {
        WS2812_SetPixel(indices[i], r, g, b);
    }
...
void jihuo(void)
{
    ArmorLED_ShowIndices(g_target_pattern_leds, ArmorLED_GetTargetPatternCount(), LED_R, LED_G, LED_B);
}
...
void daijida(void)
{
    ...
    for (uint16_t i = 0; i < ArmorLED_GetPendingPatternCount(); ++i) {
        WS2812_SetPixel(g_pending_pattern_leds[i], LED_R, LED_G, LED_B);
    }

    for (uint16_t i = STRIP2_START_INDEX; i <= STRIP3_END_INDEX; ++i) {
        WS2812_SetPixel(i, LED_R, LED_G, LED_B);
    }
